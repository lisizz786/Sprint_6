...
from src.locators import ScooterOrderPageLocator as Locators
import allure
...
class ScooterOrderPage(BasePage):
    ...
    @allure.step('Переход на следующий этап заказа')
    def go_next(self):
    ...
    @allure.step('Нажатие на заказать')
    ...
    @allure.step('Показать номер заказа')
    ...
    @allure.step('Заполнение данных "Для кого самокат"')
    ...
    @allure.step('Заполнение данных "Про аренду"')
